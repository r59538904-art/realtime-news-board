...
const SOURCES = [
  ...
  },
  {
    ...
    "lang": "JA",
    "color": "#1d9bf0",
    "note": "通常のRSSではなくX公式埋め込みウィジェットの公開エンドポイント(syndication.twitter.com)経由。ログイン・Cookie不要。リツイートは除外。このエンドポイントは期間に関係なく常に最新約20件(+固定ポスト)しか返さず、カーソル等のページング手段も存在しないことを実測で確認済み(2026-07: レスポンス内にカーソル類ゼロ、count系パラメータは全て無視、widgets.js自体にページング処理なし、旧max_position対応エンドポイントは廃止済み)。日経のような高頻度アカウントでは1回の取得では実質3〜4時間分しか取れないが、fetch_news.py側で毎回の取得結果を前回分と統合(リンクで重複排除、maxAgeMs超過分は破棄)しているため、5分おきの定期実行が積み重なることでmaxAgeMs(1日)いっぱいまでカバレッジが徐々に広がっていく(起動直後やActions初回実行直後は3〜4時間分から始まる)。これ以上1回の取得で一気に遡ることは、有料の公式APIか個人アカウントのログインなしには不可能。非公開の内部エンドポイントのため仕様変更で取得できなくなる可能性がある"
  }
];
