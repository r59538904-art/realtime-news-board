...
| `watchlist.js` | Finnhub銘柄検索・株価現在値カード(記事一覧左のサイドバー。TradingViewには依存しない。米国上場銘柄のみ対応。calendar.jsと対称のUI設計・折りたたみ対応。**利用にはFinnhubの無料APIキー設定が必要、下記参照**) |
...
- **マーケット情報の併記**: TradingViewティッカーで為替・主要株価指数を、記事一覧左の株価現在値カードで企業名検索(Finnhub API。初期表示はApple)から現在値・前日比・当日値幅を、記事一覧右の経済指標カレンダーで雇用統計・CPI・政策金利など重要指標の発表予定(重要度フィルター付き)を同一画面に表示。株価現在値カード・カレンダーともに折りたたみ対応で、ワイド画面(1840px以上)ではコンテンツ左右外側の余白に固定表示される
...
### 株価現在値カード(Finnhub APIキー)のセットアップ

記事一覧左のパネル(銘柄検索・現在値表示)はFinnhubの無料APIのみで実装しており、TradingViewには
依存していません。当初はTradingViewのチャートウィジェット(内蔵検索機能付き)を使っていましたが、
(1) 検索機能がアカウント登録・課金を求めるダイアログを出すこと、(2) Finnhubの無料プランは
過去の値動き(ローソク足チャート用のヒストリカルデータ)を提供しないこと、の2点が分かったため、
最終的に「折れ線チャートではなく現在値カード(現在値・前日比・当日値幅)」という形に落ち着いています。
以下の手順でAPIキーを設定してください(未設定でも他の機能には影響ありません。カードが
「設定が必要です」と表示されるだけです)。

1. [Finnhub](https://finnhub.io/register)で無料アカウントを作成(クレジットカード不要)
2. ダッシュボードでAPIキーを取得
3. `watchlist.js` 冒頭の `const FINNHUB_API_KEY = '';` にそのキーを入力する

無料枠は60リクエスト/分とかなり余裕があり、個人サイトでの利用には十分です。
このキーは静的サイトのJSにそのまま埋め込むため誰でも閲覧できます(view-sourceで見えます)。
Finnhub無料枠のキーはブラウザから直接叩く用途を前提にしているため実害は小さいですが、
心配な場合はFinnhub側にリファラー制限等の設定機能がないか確認してください。

#### 対応市場は米国上場銘柄のみ(既知の制約)

現在値取得(`/quote`)はFinnhub無料プランでは米国上場銘柄限定です。東証・LSE等の海外取引所は
`{"error":"You don't have access to this resource."}`が返り取得できません(検索`/search`自体は
海外取引所の銘柄もヒットするため、選んだ後に初めて取得失敗が分かる形になります。そのため
検索結果自体を米国上場銘柄だけに絞り込んで表示しています)。

無料で使えるブラウザ直叩き可能な代替を探しましたが、実機検証の結果いずれも解決しませんでした。

- **Twelve Data**: 無料プランで同じ制約を確認("This symbol is available starting with the
  Grow or Venture plan"とAPIが明示。ロンドン証券取引所ですら無料プランでは拒否される)
- **Yahoo Finance**(非公式エンドポイント): `Access-Control-Allow-Origin`ヘッダーが無く、
  ブラウザから直接fetchできない(プロキシサーバーが必要=ビルド不要構成と相容れない)
- **Stooq**: 無料CSV APIのエンドポイント自体が404になっており動作していない

海外取引所のデータ利用にはライセンス費用がかかるため無料プランでは提供されないのが
業界共通の制約のようで、追加の無料代替手段は今のところ見つかっていません。
トヨタの`TM`(NYSE上場のADR)のように、海外企業でも米国預託証券(ADR/ADS)があれば
検索・取得できる場合があります。将来的に有料プランへの切り替えや、サーバー経由の
プロキシ実装(GitHub Actions等で定期取得しstatic JSON化する、news.jsonと同じ方式)を
検討する余地はあります。
