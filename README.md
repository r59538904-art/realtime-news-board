世界ニュース速報
リアルタイム経済ニュースボード
Financial Times、CNBC、NHK、Nikkei Asia、Investing.com、FRB/ECB/日銀/BOE公式発表、FXStreet など約38の投資/金融専門RSSフィードを集約する経済ニュースボードです。RSS取得はGitHub Actions上のPythonが定期的に行い、結果を静的JSONとしてコミットする方式のため、ブラウザ側(index.html)はビルド不要のまま動作します。

## アーキテクチャ
1. `.github/workflows/fetch-news.yml`(GitHub Actions、5分おきcron)が `scripts/fetch_news.py` を実行
2. `fetch_news.py` が `sources.json` の38ソースを直接fetchし(CORSプロキシを使わないサーバー側取得)、`news.json` と `sources.js` を生成してコミット
...
主な機能
約38の投資/金融専門RSSフィードを横断して収集(為替・政策金利・商品・国際情勢に強いソースを重視)
