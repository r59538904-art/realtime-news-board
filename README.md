...
| `watchlist.js` | TradingView株価チャート+Finnhub銘柄検索(記事一覧左のサイドバー。calendar.jsと対称の設計・折りたたみ対応。**利用にはFinnhubの無料APIキー設定が必要、下記参照**) |
...
- **マーケット情報の併記**: TradingViewティッカーで為替・主要株価指数を、記事一覧左の株価チャートパネルで企業名検索(Finnhub API。初期表示はApple)からのチャート表示を、記事一覧右の経済指標カレンダーで雇用統計・CPI・政策金利など重要指標の発表予定(重要度フィルター付き)を同一画面に表示。株価チャート・カレンダーともに折りたたみ対応で、ワイド画面(1840px以上)ではコンテンツ左右外側の余白に固定表示される
...
### 銘柄検索機能(Finnhub APIキー)のセットアップ

記事一覧左の株価チャートパネルの検索欄(企業名から銘柄を探す機能)はFinnhubの無料APIを使っています。
TradingViewウィジェット内蔵の検索機能はアカウント登録・課金を求めるダイアログが出ることが確認できたため、
代わりにこの方式を採用しています。以下の手順でAPIキーを設定してください(未設定でも他の機能には影響ありません。
検索欄が「設定が必要です」と表示されるだけです)。
