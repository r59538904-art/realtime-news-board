...
// 実際のRSS取得はGitHub Actions上のPythonスクリプト(scripts/fetch_news.py)が5分おきに行い、
...
const REFRESH_MS = 1*60*1000;       // news.json再取得間隔(1分)。実データはActions側で5分おきにしか
                                     // 更新されないが、取得コストが軽い(同一オリジンのJSON1本)ため
                                     // 短めに設定し、更新が反映されるまでの体感待ち時間を減らしている
const MAX_DISPLAY = 500;            // 一覧表示件数の上限(全ソース×最大100件+4日鮮度で絞込通過後は
                                     // 400件近くになることがあるため、300では切り捨てが発生していた)
const MAX_AGE_MS = 4*24*60*60*1000; // 表示する記事の鮮度上限(4日以内)。配信日時が不明な記事は除外しない。
                                     // FRB/ECB公式発表やEconomist・日経クロステック等は更新頻度が
                                     // 2〜3日おきのことがあり、2日だと記事が0件になる時間帯が生じたため
                                     // 4日に緩和した(高頻度ソースの体感には影響しない)
